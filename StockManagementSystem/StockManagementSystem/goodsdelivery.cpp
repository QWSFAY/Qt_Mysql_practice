	...
	: QWidget(parent)
{
	...
	QPushButton* btn_add = new QPushButton("出库商品");
	...
	connect(btn_add, &QPushButton::clicked, this, &goodsdelivery::on_btn_add_clicked);
	connect(btn_cancel, &QPushButton::clicked, this, &goodsdelivery::on_btn_cancel_clicked);
...
void goodsdelivery::on_btn_add_clicked()
{
	// 获取Combo Box控件里面的值
	...
	if (inputamount > tableamount)
	{
		QMessageBox::critical(this, "提示", "对不起，商品出库的数量不能大于库存数量，请重新检查？");
		edit_nub->setFocus();
		return;
	}
...
void goodsdelivery::on_btn_cancel_clicked()
{
	close();
}
